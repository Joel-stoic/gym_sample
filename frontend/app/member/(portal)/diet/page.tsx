...
const mealConfig: Record<string, { color: string; bg: string; border: string }> = {
  ...
  'Pre-workout': { color: 'text-crayola', bg: 'bg-crayola-100', border: 'border-crayola/20' },
...
const defaultMeal = { color: 'text-gunmetal-400', bg: 'bg-zinc-500/10', border: 'border-zinc-500/20' }
const card = 'rounded-2xl border border-white/[0.08] bg-white/[0.03] p-5'
const inputBase = 'w-full bg-black/30 border border-white/[0.08] rounded-xl px-3 py-2.5 text-sm text-white placeholder:text-zinc-600 outline-none focus:border-crayola/40 transition-colors'

function Label({ children }: { children: React.ReactNode }) {
  return <p className="text-[10px] font-semibold uppercase tracking-[0.15em] text-gunmetal-400 mb-3">{children}</p>
}
  ...
  return (
    <div className="rounded-xl border border-white/[0.07] bg-black/20 overflow-hidden">
      ...
      >
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className={cn(
              ...
              enrollment.status === 'ACTIVE'
                ...
                : 'text-gunmetal-400 bg-zinc-500/10 border-zinc-500/20'
            )}>
              {enrollment.status}
            </span>
          </div>
          ...
          <div className="flex items-center gap-2 mt-2">
            <div className="flex-1 h-1.5 bg-zinc-800 rounded-full overflow-hidden">
              <div
                className="h-full bg-crayola rounded-full transition-all"
                style={{ width: `${pct}%` }}
              />
            </div>
            <span className="text-[11px] text-gunmetal-400 flex-shrink-0">
              {enrollment.usedSessions}/{enrollment.totalSessions} sessions
            </span>
          </div>
          <div className="flex items-center gap-3 mt-1.5">
            <span className="text-[11px] text-crayola font-medium">
        ...
        {open
          ? <ChevronUp className="h-3.5 w-3.5 text-gunmetal-400 flex-shrink-0 mt-1" />
          : <ChevronDown className="h-3.5 w-3.5 text-gunmetal-400 flex-shrink-0 mt-1" />
        }
      </button>
      ...
      {open && (
        <div className="px-4 pb-4 border-t border-white/[0.05] pt-3 space-y-3">
          ...
          {upcoming.length > 0 && (
            <div>
              <p className="text-[11px] font-semibold text-gunmetal-400 uppercase tracking-wider mb-2">
                Upcoming
              </p>
                  ...
                  return (
                    <div key={session.id} className="flex items-center gap-3 rounded-xl border border-blue-500/15 bg-blue-500/5 px-3 py-2.5">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[11px] text-gunmetal-400 flex items-center gap-1 mt-0.5">
                  ...
                  return (
                    <div key={session.id} className={cn(
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[11px] text-gunmetal-400">{session.trainer.name}</p>
                      </div>
      ...
      >
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1.5">
            <span className="text-[11px] text-gunmetal-400 flex items-center gap-1">
        ...
        <div className="flex items-center gap-2 pt-0.5 flex-shrink-0">
          ...
          {open ? <ChevronUp className="h-3.5 w-3.5 text-gunmetal-400" /> : <ChevronDown className="h-3.5 w-3.5 text-gunmetal-400" />}
        </div>
      </button>

      {open && (
        <div className="px-4 pb-4 space-y-2.5 border-t border-white/[0.05] pt-3">
          {plan.description && <p className="text-xs text-gunmetal-400 italic mb-3">{plan.description}</p>}
          {plan.meals.map(meal => {
              ...
              <div key={meal.id} className="rounded-xl border border-white/[0.05] bg-black/20 p-3">
                ...
                <ul className="space-y-1.5">
                  {meal.items.map((item, i) => (
                    <li key={i} className="text-xs text-gunmetal-100 flex items-center gap-2">
          ...
          {plan.notes && (
            <p className="text-xs text-gunmetal-400 border-t border-white/[0.05] pt-3 mt-1">
              <span className="text-gunmetal-400 font-medium">Trainer note:</span> {plan.notes}
            </p>
          )}
        </div>
      )}
    ...
    <div className={card}>
      ...
      <div className="space-y-3">
        <div className="relative">
          ...
          />
          <span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm text-gunmetal-400 font-medium select-none">kg</span>
        </div>
        {error && (
...
const setData =
  ...
  const changeColor = change === null ? 'text-gunmetal-400' : change < 0 ? 'text-emerald-400' : change > 0 ? 'text-red-400' : 'text-gunmetal-400'
  ...
  return (
    <div className="space-y-5 pb-10">
      ...
      <div>
        ...
        <p className="text-xs text-gunmetal-400 mt-0.5">Track your progress and follow your trainer's plan</p>
      </div>
          ...
          <div className={card}>
            ...
            <div className="grid grid-cols-3 gap-2 mb-4">
              <div className="rounded-xl bg-black/20 border border-white/[0.06] p-3 text-center">
                ...
                <p className="text-[10px] text-gunmetal-400">Now (kg)</p>
              </div>
              <div className="rounded-xl bg-black/20 border border-white/[0.06] p-3 text-center">
                <p className="text-xl font-semibold text-white leading-none mb-1">{summary?.oldest ?? '—'}</p>
                <p className="text-[10px] text-gunmetal-400">Start (kg)</p>
              </div>
              <div className="rounded-xl bg-black/20 border border-white/[0.06] p-3 text-center">
                ...
                <p className="text-[10px] text-gunmetal-400">
                ...
                {entries.map((entry: WeightEntry, idx: number) => {
                  ...
                  return (
                    <div key={entry.id} className={cn(
                      ...
                      fresh ? 'border-crayola/20 bg-crayola/5' : 'border-white/[0.05] bg-black/20'
                    )}>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-white flex items-center gap-2">
                          {entry.weight} kg
                          {fresh && <span className="text-[10px] text-crayola bg-crayola-100 border border-crayola/20 px-1.5 py-0.5 rounded">Today</span>}
                        </p>
                        <p className="text-[11px] text-gunmetal-400">{format(parseISO(entry.loggedAt), 'd MMM yyyy')}</p>
                      </div>
                      {diff !== null && diff !== 0 && (
          ...
          <div className={card}>
            ...
            {diets.length === 0 ? (
              <div className="py-12 flex flex-col items-center gap-3 text-zinc-600">
                <div className="h-12 w-12 rounded-2xl bg-crayola-100 flex items-center justify-center">
                  <Utensils className="h-5 w-5 text-crayola/60" />
                </div>
                <div className="text-center">
                  <p className="text-sm text-gunmetal-400">No diet plan yet</p>
          ...
          <div className={card}>
            ...
            {ptEnrollments.length === 0 ? (
              <div className="py-12 flex flex-col items-center gap-3 text-zinc-600">
                ...
                <div className="text-center">
                  <p className="text-sm text-gunmetal-400">No PT package yet</p>
